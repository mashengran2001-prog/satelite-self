...
/** Exit IP / fraud score via IPPure for the enabled nodes (requires a running
 * generated sing-box / mihomo core with manual selection). */
export function testNodesIppure(ids?: string[] | null) {
  return invoke<IppureBatchResult>("test_nodes_ippure", {
    ids: ids ?? null,
  });
}

/** Ask a running IPPure batch to stop; already-tested nodes keep their results. */
export function cancelIppureProbe() {
  return invoke<void>("cancel_ippure_probe");
}
