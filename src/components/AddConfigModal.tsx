...
export function AddConfigModal({
  ...
  const [subInputMode, setSubInputMode] = useState<"url" | "file">("url");
  ...
  useEffect(() => {
    ...
    if (initial) {
      ...
      if (initial.autoUpdate === false) {
        setAutoUpdateInterval("disabled");
      } else if (interval === 60) {
        setAutoUpdateInterval("1h");
      } else if (interval === 720) {
        setAutoUpdateInterval("12h");
      } else if (interval === 1440) {
        setAutoUpdateInterval("24h");
      } else {
        setAutoUpdateInterval("custom");
        setCustomIntervalMin(String(interval));
      }
    } else {
  ...
  function currentKind(): AddSourceKind {
    if (profile === "subscription") return subInputMode === "file" ? "text" : "url";
  ...
  function handleSubmit(e: FormEvent) {
    ...
    const parsedCustomMin = Math.floor(Number(customIntervalMin));
    const interval = autoUpdate
      ? autoUpdateInterval === "custom"
        ? Math.min(
            10080,
            Math.max(1, Number.isFinite(parsedCustomMin) ? parsedCustomMin : 60),
          )
    ...
    if (kind === "url") {
      const localPath = looksLikeLocalPath(url) ? url.trim() : undefined;
      if (localPath) {
        payload.kind = "file";
  ...
  const localPathUrl = kind === "url" && looksLikeLocalPath(normalizedUrl);
  const duplicateUrl =
    kind === "url" &&
    !localPathUrl &&
            ...
            <>
              <div className="field">
                ...
                <GlassSeg
                  ...
                  ariaLabel="订阅输入方式"
                  disabled={busy}
                  onChange={(v) => setSubInputMode(v as "url" | "file")}
                  options={[
                    ...
                    {autoUpdateInterval === "custom" && (
                      <div className="file-row">
                        <input
                          ...
                          disabled={busy}
                          aria-label="自定义自动更新间隔（分钟）"
                          placeholder="分钟"
                        />
                        <span className="field-hint muted">分钟</span>
                      </div>
                    )}
                  </div>
                </>
                ...
                <>
                  <div className="field">
                    ...
                    <div className="file-row">
                      <input
                        readOnly
                        value={fileLabel}
                        placeholder="选择 Clash YAML / 订阅文件"
                        disabled={busy}
                      />
                      <button
                        type="button"
                  ...
                  <label className="field">
                    ...
                    <textarea
                      ...
                      placeholder="选择文件后会自动拷贝进应用，也可直接粘贴 Clash YAML / 协议链接"
                      disabled={busy}
                      rows={8}
                    />
                  </label>
