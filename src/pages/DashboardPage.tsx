...
export function DashboardPage({
  ...
  const [ippureProbing, setIppureProbing] = useState(false);
  const [ippureResult, setIppureResult] = useState<IppureResult | null>(null);
...
function coreDisplayName(kind: string | null | undefined): string {
  ...
  useEffect(() => {
    setIppureResult(getIppureResult(currentNode?.id) ?? null);
  }, [currentNode?.id]);

  useEffect(() => {
    let unlisten: (() => void) | undefined;
    let cancelled = false;
    void listen<IppureResult>("ippure-progress", (event) => {
      const r = event.payload;
      rememberIppureResult(r);
      if (currentNode?.id && r.id === currentNode.id) {
        setIppureResult(r);
      }
    })
      .then((stop) => {
  ...
  async function onProbeIppure() {
    if (!currentNode || ippureProbing || !running) return;
    setIppureProbing(true);
    setError(null);
    try {
      const batch = await testNodesIppure([currentNode.id]);
      const r = batch.results.find((item) => item.id === currentNode.id);
      if (r) {
        rememberIppureResult(r);
        setIppureResult(r);
      }
  ...
  const currentIppureNature = ippureNatureKey(ippureResult);
  const currentIppureNatureLabel =
    currentIppureNature === "residential"
      ? t("nodes.ippureResidential")
      : currentIppureNature === "datacenter"
        ? t("nodes.ippureDatacenter")
        : currentIppureNature === "broadcast"
      ...
      <section className="instrument-grid instrument-grid-6" aria-label="Telemetry">
        ...
        >
          ...
          <div className="instrument-kv mono">
            ...
            <div className="instrument-ippure-row">
              <span className="kv-k">{t("dashboard.ippure")}</span>
              <span
                className="instrument-ippure-value"
                title={
                  [currentIppureNatureLabel, ippureResult?.as_organization]
                    .filter(Boolean)
                    .join(" · ") || t("dashboard.ippureTest")
                }
              >
                {ippureProbing ? (
                  <span
                    className="lat-spinner"
                    aria-label={t("dashboard.ippureTesting")}
                  />
                ) : ippureResult ? (
                  <>
                    <IppureDisplay result={ippureResult} testing={false} compact />
                    {currentIppureNatureLabel ? (
                      <span
                        className={`ippure-nature ippure-nature-${currentIppureNature}`}
                      >
                        {currentIppureNatureLabel}
                      </span>
                    ) : null}
                  ...
                  <span className="ippure ippure-none">
                    {t("nodes.ippureUntested")}
                  </span>
                )}
              </span>
              <button
                type="button"
                className="instrument-ippure-btn"
                disabled={ippureProbing || !running || !currentNode}
                title={t("dashboard.ippureTest")}
                aria-label={t("dashboard.ippureTest")}
                onClick={(e) => {
                  e.stopPropagation();
                  void onProbeIppure();
                }}
              ...
              >
                {ippureProbing ? (
                  ...
                  <span className="ippure-btn-label">IP</span>
                )}
              </button>
            </div>
          </div>
