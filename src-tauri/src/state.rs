...
mod kernel_selection_poll_tests {
    ...
    #[test]
    fn ippure_probe_lock_does_not_block_core_transitions() {
        let test_dir = std::env::temp_dir().join(format!(
            "satelite-ippure-lock-{}-{}",
            std::process::id(),
            std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .expect("system clock")
        ...
        let state = AppState::load(test_dir.clone(), None).expect("load test state");
        let probe = state.begin_ippure_probe().expect("start purity probe");
        assert!(
            !state.is_core_transitioning(),
            "purity probe must not claim the core transition flag"
        );
        assert!(
            !state.ippure_cancel.load(Ordering::Acquire),
            "fresh batch starts uncancelled"
        );
        assert!(state.begin_ippure_probe().is_err(), "duplicate probe rejected");
        drop(probe);
        let probe = state.begin_ippure_probe().expect("probe reusable after drop");
        assert!(state.begin_ippure_probe().is_err());
        drop(probe);
        state.cancel_ippure_probe();
        assert!(state.ippure_cancel.load(Ordering::Acquire), "cancel flag set");
        let probe = state.begin_ippure_probe().expect("new batch resets cancel");
        assert!(
            !state.ippure_cancel.load(Ordering::Acquire),
            "new batch clears cancel"
        );
        drop(probe);
        let _ = std::fs::remove_dir_all(test_dir);
    }
...
pub struct AppState {
    ...
    /// True while an IPPure batch probe runs. Deliberately separate from
    /// `core_transitioning`: a long purity batch must not block unrelated core
    /// actions with "内核正在切换，请稍候" for the whole run.
    ippure_probe_running: AtomicBool,
    /// Set when the user asks to stop the running IPPure batch.
    ippure_cancel: AtomicBool,
...
pub struct IppureProbeGuard<'a> {
    flag: &'a AtomicBool,
}

impl Drop for IppureProbeGuard<'_> {
    fn drop(&mut self) {
        self.flag.store(false, Ordering::Release);
    }
}
...
impl AppState {
    pub fn load(app_data_dir: PathBuf, resource_dir: Option<PathBuf>) -> AppResult<Self> {
        ...
        Ok(Self {
            ...
            ippure_probe_running: AtomicBool::new(false),
            ippure_cancel: AtomicBool::new(false),
    ...
    pub fn begin_core_transition(&self) -> AppResult<CoreTransitionGuard<'_>> {
    ...
    /// IPPure batches only prevent another concurrent probe. They must not take
    /// the core transition flag, because a long batch would make unrelated
    /// actions report "内核正在切换" for the whole run.
    pub fn begin_ippure_probe(&self) -> AppResult<IppureProbeGuard<'_>> {
        self.ippure_probe_running
            .compare_exchange(false, true, Ordering::AcqRel, Ordering::Acquire)
            .map_err(|_| crate::error::AppError::Core("已有 IP 纯净度检测正在进行".into()))?;
        self.ippure_cancel.store(false, Ordering::Release);
        Ok(IppureProbeGuard {
            flag: &self.ippure_probe_running,
        })
    }

    pub fn cancel_ippure_probe(&self) {
        self.ippure_cancel.store(true, Ordering::Release);
    }

    pub fn ippure_cancel_flag(&self) -> &AtomicBool {
        &self.ippure_cancel
    }
