...
pub async fn probe_nodes(
    ...
    probe_nodes_with_progress(nodes, timeout_ms, concurrency, clash, probe_url, |_| {}).await
}

/// Same as [`probe_nodes`], but calls `on_result` for every result as soon as
/// its probe finishes, so callers can stream per-node progress to the UI.
pub async fn probe_nodes_with_progress<F>(
    nodes: &[ProxyNode],
    timeout_ms: Option<u64>,
    concurrency: Option<usize>,
    clash: Option<ClashApi>,
...
mod tests {
    ...
    #[tokio::test]
    async fn progress_callback_receives_each_result_as_it_finishes() {
        use crate::domain::Protocol;
        let nodes = vec![node(Protocol::Shadowsocks)];
        let seen = Arc::new(Mutex::new(Vec::<String>::new()));
        let seen_in_cb = Arc::clone(&seen);
        let results = probe_nodes_with_progress(
            &nodes,
            Some(200),
            Some(1),
            None,
            String::new(),
            move |result| seen_in_cb.lock().unwrap().push(result.id.clone()),
        )
        .await
        .unwrap();
