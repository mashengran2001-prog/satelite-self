...
pub async fn import_from_url_with_id(
    ...
    if !status.is_success() {
        let message = if status.as_u16() == 407 {
            "http status 407 Proxy Authentication Required — 目标服务器或上游代理要求认证，当前应用未配置代理账号密码。请关闭“通过代理拉取”，或改用无需代理认证的订阅链接。".to_string()
        } else {
...
fn subscription_user_agent() -> String {
    ...
    "satelite-self/0.1 clash-verge/v2.5 flclash/1".to_string()
}
...
"#;
    ...
    #[test]
    fn same_name_host_port_protocol_with_different_credentials_collapses_to_one() {
        // Two entries with the same display name, host, port and protocol
        // but different credentials hash to the same node id. Keeping both
        // would make the generated sing-box config fail on duplicate
        // outbound tags, so the import keeps the first one.
        let a = "hysteria2://11111111-1111-1111-1111-111111111111@203.10.98.188:443/?insecure=false&sni=www.bing.com#HK";
        let b = "hysteria2://22222222-2222-2222-2222-222222222222@203.10.98.188:443/?insecure=false&sni=www.bing.com#HK";
        let outcome = import_from_text(Some("dup-auth".into()), format!("{a}\n{b}"), None).unwrap();
        assert_eq!(outcome.nodes.len(), 1);
        assert_eq!(outcome.subscription.node_count, 1);
        assert_eq!(outcome.nodes[0].name, "HK");
    }
}
...
fn build_outcome(
    ...
    let sub_id = id.clone();
    // Two nodes can collide on name+server+port+protocol even when their
    // credentials differ; the id hash cannot tell them apart, so keep the
    // first and drop the rest instead of writing duplicate rows that make
    // sing-box reject the generated config.
    let nodes: Vec<ProxyNode> = dedupe_by_id(
        real_nodes
            .into_iter()
            .map(|mut n| {
...
fn dedupe_by_id(nodes: Vec<ProxyNode>) -> Vec<ProxyNode> {
