...
/// `node-{id}` hashes (`config::builder::outbound_tag`), not display
...
/// embedded in the tag. Legacy configs from older builds used the first 16
/// hex chars, so a short suffix still matches via its id prefix. Ids are
/// left untouched so latency results stay stable across calls.
fn restore_generated_tag_names(nodes: &mut [ListedNode], prefix_names: &HashMap<String, String>) {
    for listed in nodes.iter_mut() {
        ...
        let display = prefix_names
            .get(suffix)
            .or_else(|| prefix_names.get(&suffix[..suffix.len().min(16)]));
...
mod tests {
    ...
    fn restore_generated_tag_names_recovers_display_names() {
        // Round-trip: a generated-style config carries internal `node-{id}`
        // tags; the display name is recovered via the embedded id.
        ...
        map.insert(id.clone(), "香港 01".to_string());
