...
pub async fn add_subscription_url(
    ...
    // Pasting a local Clash YAML path into the URL field is a common mistake;
    // route it to the same file import path instead of failing on "must start
    // with http://".
    if is_local_file_input(&url) {
        let path = local_file_path_from_input(&url).ok_or_else(|| {
            format!("检测到本地文件路径，但文件不存在：{url}；请确认路径后再导入")
        })?;
        let outcome = import_file_blocking(name, path, None).await?;
        return persist_import(&app, &state, outcome);
    }
...
fn is_local_file_input(input: &str) -> bool {
...
fn local_file_path_from_input(input: &str) -> Option<PathBuf> {
    let value = input.trim();
    if value.to_ascii_lowercase().starts_with("file://") {
...
mod local_path_tests {
    ...
    #[test]
    fn detects_drive_unc_and_file_urls_as_local_input() {
        assert!(is_local_file_input("D:\\edge\\iKuuu_V2.yaml"));
        assert!(is_local_file_input("d:/edge/iKuuu_V2.yaml"));
        assert!(is_local_file_input("\\\\nas\\share\\clash.yaml"));
        assert!(is_local_file_input("file:///D:/edge/iKuuu_V2.yaml"));
        assert!(!is_local_file_input("https://example.com/sub"));
        assert!(!is_local_file_input(""));
    }

    #[test]
    fn resolves_existing_file_url_to_a_path() {
        ...
        let file = dir.join("clash.yaml");
        std::fs::write(&file, b"proxies: []").unwrap();

        let drive_input = file.to_string_lossy().replace('/', "\\");
        assert_eq!(local_file_path_from_input(&drive_input), Some(file.clone()));

        let file_url = url::Url::from_file_path(&file).unwrap();
        assert_eq!(local_file_path_from_input(file_url.as_str()), Some(file));

        assert!(local_file_path_from_input("Z:\\definitely\\missing.yaml").is_none());
