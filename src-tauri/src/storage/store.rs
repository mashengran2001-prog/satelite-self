...
impl AppStore {
    ...
    pub fn enabled_nodes(&self) -> Vec<ProxyNode> {
        let enabled: std::collections::HashSet<_> = self
            ...
            .map(|s| s.id.as_str())
            .collect();
        // Duplicate rows for one node id (stale stores, or two subscriptions
        // containing identical nodes) must collapse to one selectable node.
        let mut seen = std::collections::HashSet::new();
        self.nodes
            .iter()
            .filter(|n| enabled.contains(n.subscription_id.as_str()))
            .filter(|n| seen.insert(n.node.id.as_str()))
            .map(|n| n.node.clone())
            .collect()
    }
...
mod tests {
    ...
    #[test]
    fn enabled_nodes_dedupe_duplicate_rows_with_the_same_id() {
        let mut store = AppStore::default();
        let sub = sample_url_sub("a");
        let sub_id = sub.id.clone();
        store.subscriptions.push(sub);
