...
- `lib.rs` — `run()`：插件注册（opener/dialog/deep-link/single-instance）→ setup（加载 store 失败则弹窗退出）→ 托盘 → 启动 6 个后台任务 → 深链处理 → 静默启动/自动代理恢复。**全部 ~81 个 command 在 `lib.rs:348-431` 注册**，实现在 `commands/*.rs`（`commands/mod.rs` re-export）。
...
- `services/ippure.rs` — IPPure 纯净度检测：经本地 mixed 端口请求 IPPure 接口，临时把主 `proxy` selector 切到 `node-<id>`、等切换稳定后探测退出 IP / fraudScore / 风险分级 / 归属地，逐节点还原原选择（批量失败也还原）；命令层 `commands/ippure.rs` 拒绝 custom sing-box、Xray 与 kernel-auto 模式。
...
`config.rs`（订阅 CRUD/激活/mix、`generate/preview_singbox_config` 按 core_type 分发三生成器，mihomo 返回 YAML 文本；节点列表按 `CoreKind::supports_node` 过滤）、`core.rs`（启停/重启/capture_mode/三内核下载更新/`set_core_type` 切内核/`refresh_geodata` 带 kind 参数——xray 刷 Loyolsoldier .dat、mihomo 刷 MetaCubeX mmdb/GeoSite.dat）、`connections.rs`（连接/请求/失败；`list_connection_changes` 增量协议：带 `lastOrderRevision`，纯计数更新不下发 `order_ids`）、`diagnostics.rs`、`dns.rs`（DNS+hosts）、`ippure.rs`（`test_nodes_ippure`：IPPure 纯净度批量检测，需运行中的 sing-box/mihomo + 手动模式）、`latency.rs`、`logs.rs`、`proxy.rs`（状态/系统代理/TUN）、`rules.rs`（规则集 CRUD/排序/远程规则，1167 行）、`subscription.rs`（导入各来源）。command 名与 `src/api.ts` 导出一一对应（snake_case）。
...
| `NodesPage` | 列表/网格（`useVirtualRange`×2）、搜索排序测速、IPPure 纯净度检测、改名、custom 配置节点；切节点 `waitForCoreRestart` |
...
| `SettingsPage` (1456) | 6 tab：app/ports/rules/dns/hosts/core；内嵌 Rules/Dns/Hosts 页；三内核行（各自版本/下载/更新，进度事件按 kind 分流）、更新检查、诊断、托盘图标选择 |
...
`SimpleShell`（4 tab：connect/servers/traffic/settings）+ 各页。复用玻璃设计语言与 `AddConfigModal`；`SimpleTrafficSpark` 为 SVG 迷你流量图；`SimpleServersPage` 含测速与 IPPure 纯净度检测入口。新增面向普通用户的轻量入口时改这里。
...
| 改 IP 纯净度检测 | `src-tauri/src/services/ippure.rs` + `commands/ippure.rs` + 页面按钮（NodesPage / SimpleServersPage）+ `i18n/messages.ts` 双语 |
